...
export function ShareWithFriendsModal({ tableId, isOpen, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [friendsRes, sentRes] = await Promise.all([
        ...
        authedFetch(`/api/invites?sent=true&tableId=${tableId}`),
      ])
  ...
  const handleSend = async () => {
    ...
    try {
      await authedFetch('/api/invites', {
        method: 'POST',
        body: JSON.stringify({ tableId, toUids: Array.from(selected) }),
      })
      Analytics.inviteSent({ table_id: tableId, count: selected.size })
