...
export function FriendsCarousel({ tables, recaps = [] }: FriendsCarouselProps) {
  ...
  // Build table items, one per friend (soonest upcoming first)
  const seenUids = new Set<string>()
  const tableItems: CarouselItem[] = tables
    .slice()
    ...
    .map((e) => ({ type: 'table', table: e }))

  // Recap items: only for friends without an upcoming table bubble, most recent first
  ...
  const items = [...tableItems, ...recapItems]
    ...
    <>
        ...
        <div className="flex gap-4 w-max py-2">
          {items.map((item, i) => {
            if (item.type === 'table') {
              ...
              const ongoing = getEffectiveStatus(table) === 'ongoing'
              const isPrivate = table.type === 'private'
              return (
                <button
                  key={table.id}
                  onClick={() => { setSelectedIndex(i); Analytics.carouselTapped({ table_id: table.id, game: table.boardGame.name, status: getEffectiveStatus(table) }) }}
      ...
      {selectedIndex !== null && (() => {
        ...
        return item.type === 'table'
          ? <TablePreviewModal table={item.table} {...nav} />
...
function TablePreviewModal({ table, index, total, onPrev, onNext, onClose }: {
  table: GameTable
  index: number
  total: number
  onPrev: () => void
    ...
    >
      ...
      >
        ...
        <div className="px-5 pt-4 pb-4">
          ...
          <h2 className="text-xl font-extrabold text-on-surface pr-8 tracking-[-0.02em]">{table.boardGame.name}</h2>
        </div>
        ...
        <div className="px-5 py-4 space-y-3.5">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-on-surface leading-snug font-meta">{formatDateTime(table.dateTime, i18n.language)}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-on-surface leading-snug font-meta">{table.addressLabel ?? table.address}</p>
              {table.addressLabel && <p className="text-xs text-on-surface-variant/50 font-meta mt-0.5">{table.address}</p>}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-on-surface font-meta">
                ...
                <span className="text-on-surface-variant/50 font-normal"> · min {table.minPlayers}</span>
              </p>
            </div>
          </div>
        </div>
