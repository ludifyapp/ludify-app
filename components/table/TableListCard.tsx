...
import { GameTable, Player } from '@/types'
import { formatDateTime, getEffectiveStatus } from '@/lib/utils'
import { TableStatusBadge } from './TableStatusBadge'

export function TableListCard({ table, friendsInTable }: { table: GameTable; friendsInTable?: Player[] }) {
    ...
    <Link href={`/table/${table.id}`}>
      <div className="group bg-surface-container-high rounded-[1.5rem] hover:bg-surface-container-highest transition-colors duration-200 overflow-hidden flex gap-0 cursor-pointer">
        ...
        <div className="flex-shrink-0 w-36 self-stretch relative">
          {table.boardGame.thumbnail && !imgError ? (
            ...
            <div className="absolute inset-0 bg-surface-container-highest rounded-l-[1.5rem] flex items-center justify-center">
              <span className="text-2xl font-bold text-teal-600">{table.boardGame.name.charAt(0).toUpperCase()}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 p-5 flex flex-col justify-center gap-2.5">
          <div className="flex items-start gap-3 w-full">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-1">
                <h3 className="text-base font-bold text-on-surface leading-tight tracking-[-0.02em]">{table.boardGame.name}</h3>
                <TableStatusBadge status={effectiveStatus} />
              </div>
              <p className="text-sm text-on-surface-variant/70 font-meta">{formatDateTime(table.dateTime, i18n.language)}</p>
              <p className="text-sm text-on-surface-variant/60 font-meta truncate mt-0.5">{table.addressLabel ?? table.address}</p>
              {host && (
                <div className="flex items-center gap-1.5 mt-2">
                  ...
                  <p className="text-xs text-on-surface-variant/60 font-meta">{t('tableCard.by', { name: host.name })}</p>
                </div>
              )}
            </div>
          ...
          {/* Friends who joined */}
          {friendsInTable && friendsInTable.length > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex -space-x-1.5 flex-shrink-0">
                {friendsInTable.slice(0, 3).map((friend, i) => (
              ...
              <p className="text-xs text-on-surface-variant/60 font-meta truncate">
                {friendsInTable.length === 1 && <span className="font-medium text-on-surface">{friendsInTable[0].name.split(' ')[0]}</span>}
                {friendsInTable.length === 2 && <span><span className="font-medium text-on-surface">{friendsInTable[0].name.split(' ')[0]}</span> & <span className="font-medium text-on-surface">{friendsInTable[1].name.split(' ')[0]}</span></span>}
                {friendsInTable.length > 2 && <span><span className="font-medium text-on-surface">{friendsInTable[0].name.split(' ')[0]}</span>, <span className="font-medium text-on-surface">{friendsInTable[1].name.split(' ')[0]}</span> +{friendsInTable.length - 2}</span>}
                {' '}{t('home.joinedTable', 'joined')}
              </p>
            </div>
          )}
        </div>
