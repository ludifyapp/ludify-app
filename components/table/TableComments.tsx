...
export function TableComments({ tableId, hostUid, hostName, canComment, allowComments = true }: TableCommentsProps) {
  ...
  useEffect(() => {
    const q = query(collection(db, 'tables', tableId, 'comments'), orderBy('createdAt', 'desc'))
    return onSnapshot(q, (snap) => {
      setComments(snap.docs.map((d) => ({ id: d.id, ...d.data() } as Comment)))
    })
  }, [tableId])

  // Fetch mute status for this table
  useEffect(() => {
    if (!user) return
    const unsub = onSnapshot(doc(db, 'users', user.uid), (snap) => {
      const mutedTables: string[] = snap.data()?.mutedTables ?? []
      setMuted(mutedTables.includes(tableId))
    })
    return unsub
  }, [user, tableId])
  ...
  const toggleMute = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/tables/${tableId}/mute`, {
        method: 'POST',
  ...
  const post = async () => {
    ...
    try {
      ...
      if (res.ok) {
        Analytics.commentPosted({ table_id: tableId })
        setText('')
        setFocused(false)
      }
    } finally {
  ...
  const deleteComment = (id: string) => {
    Analytics.commentDeleted({ table_id: tableId })
    return deleteDoc(doc(db, 'tables', tableId, 'comments', id))
  }
  const togglePin = (comment: Comment) => {
    if (!comment.pinned) Analytics.commentPinned({ table_id: tableId })
    return updateDoc(doc(db, 'tables', tableId, 'comments', comment.id), { pinned: !comment.pinned })
  }
  const editComment = (id: string, newText: string) => {
    return updateDoc(doc(db, 'tables', tableId, 'comments', id), {
      text: newText,
      isEdited: true
    })
  }
  ...
  const toggleReaction = async (commentId: string, emoji: string) => {
    ...
    fetch(`/api/tables/${tableId}/comments/${commentId}/reactions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ emoji }),
    })
      .then(() => { if (!wasReacted) Analytics.reactionAdded({ table_id: tableId, emoji }) })
      .catch(() => {})
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {canComment ? (
        ...
        <p className="text-sm text-slate-400 dark:text-zinc-500 italic">
          {!allowComments ? t('comments.disabled', 'Comments are disabled for this table') : t('comments.joinToComment')}
        </p>
      )}
