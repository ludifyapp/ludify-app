...
export function HostRatingForm({ tableId, hostName }: HostRatingFormProps) {
  ...
  useEffect(() => {
    auth.currentUser?.getIdToken().then((token) => {
      fetch(`/api/tables/${tableId}/rating`, {
  ...
  const submit = async (score: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/tables/${tableId}/rating`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        Analytics.hostRated({ table_id: tableId, score })
      }
    } finally {
      setSubmitting(false)
    }
