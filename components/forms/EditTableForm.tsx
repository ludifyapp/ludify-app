...
interface EditTableFormProps {
  table: GameTable
  onSave: (data: Partial<GameTable>) => Promise<void>
  onClose: () => void
}

export function EditTableForm({ table, onSave, onClose }: EditTableFormProps) {
  const { t } = useTranslation()
  const [gameName, setGameName] = useState(table.boardGame.name)
  const [description, setDescription] = useState(table.description ?? '')
  const [dateTime, setDateTime] = useState(formatDateTimeInput(table.dateTime))
  const [endDateTime, setEndDateTime] = useState(table.endDateTime ? formatDateTimeInput(table.endDateTime) : '')
  const [address, setAddress] = useState(table.address)
  const [addressLabel, setAddressLabel] = useState(table.addressLabel ?? '')
  const [minPlayers, setMinPlayers] = useState(String(table.minPlayers ?? 2))
  const [maxPlayers, setMaxPlayers] = useState(String(table.maxPlayers))
  const [type, setType] = useState<'public' | 'private'>(table.type ?? 'public')
  const [errors, setErrors] = useState<Record<string, string>>({})
  const [loading, setLoading] = useState(false)
  const [allowComments, setAllowComments] = useState(table.allowComments !== false)
  ...
  const originalDateTime = formatDateTimeInput(table.dateTime)
  const minDateTime = (() => {
  ...
  const validate = () => {
    ...
    if (max < table.players.length) e.maxPlayers = `Cannot be less than current players (${table.players.length})`
    return e
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave({
        boardGame: { bggId: table.boardGame.bggId, name: gameName.trim(), thumbnail: table.boardGame.thumbnail ?? '' },
      ...
      Analytics.tableEdited({ table_id: table.id })
      onClose()
    } catch (err) {
          ...
          <div className="mt-4 p-4 bg-surface-container-high rounded-[1.5rem]">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-on-surface-variant/60">{t('manage.allowCommentsDesc', 'Let players post messages in the table')}</p>
              </div>
              <button
                type="button"
