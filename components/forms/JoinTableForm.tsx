...
export function JoinTableForm({ onJoin, status }: JoinTableFormProps) {
    ...
    return (
      <div className="bg-surface-container-high rounded-[1.5rem] p-4 text-center">
        <p className="text-on-surface font-medium">
          {status === 'ongoing' ? t('joinForm.tableOngoing') : t('joinForm.tableEnded')}
        </p>
      </div>
    )
  }
  ...
  if (status === 'cancelled') {
    return (
      <div className="bg-error-container rounded-[1.5rem] p-4 text-center">
        <p className="text-error font-medium">{t('joinForm.tableCancelled')}</p>
      </div>
    )
  }
  ...
  if (status === 'full') {
    return (
      <div className="bg-secondary-container rounded-[1.5rem] p-4 text-center">
        <p className="text-on-secondary-container font-medium">{t('joinForm.tableFull')}</p>
