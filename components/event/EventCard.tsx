...
export function EventCard({ event, backHref, manageHref }: EventCardProps) {
  ...
  const handleCopyLink = async () => {
    Analytics.shareLinkCopied({ event_id: event.id })
    try {
  ...
  const handleWhatsApp = () => {
    const text = t('share.whatsappText', { game: event.boardGame.name, url: eventUrl })
        ...
        <div className="relative w-full h-[220px] md:h-[260px] rounded-2xl overflow-hidden">
          {event.boardGame.thumbnail && !imgError ? (
            ...
            <div className="w-full h-full bg-surface-container-high flex items-center justify-center">
              <span style={{ fontSize: '5rem', fontWeight: 900, opacity: 0.15, color: 'var(--color-primary)', userSelect: 'none' }}>
                {event.boardGame.name.charAt(0).toUpperCase()}
              </span>
            </div>
          )}
          ...
          <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10">
            ...
            {manageHref && (
              ...
              >
                {t('event.manageEvent', 'Manage Event')}
              </Link>
            )}
          </div>
      ...
      <div className="max-w-lg mx-auto w-full">
        ...
        <section className={`${PAD} mt-5`}>
          ...
          {host ? (
            <p className="text-on-surface-variant font-medium">
              {t('eventCard.hostedBy', 'Hosted by')}{' '}
              <span className="text-primary font-semibold">{host.name.split(' ')[0]}</span>
            </p>
          ) : event.boardGame.yearPublished ? (
            <p className="text-on-surface-variant font-medium">{event.boardGame.yearPublished}</p>
          ) : null}
        </section>
        ...
        <section className={`${PAD} mt-6 flex gap-3`}>
          ...
          >
            ...
            <span className="text-center font-bold text-[10px] uppercase tracking-widest">{t('share.shareEvent', 'Share')}</span>
          </button>
          ...
          >
            ...
            <span className="text-center font-bold text-[10px] uppercase tracking-widest">{t('eventCard.addToCalendar', 'Calendar')}</span>
          </a>
          ...
          >
            ...
            <span className="text-center font-bold text-[10px] uppercase tracking-widest">{t('eventCard.map', 'Map')}</span>
          </button>
        </section>
        ...
        <section className={`${PAD} mt-10 flex flex-col gap-4`}>
          ...
          {event.endDateTime && !isSameDay(event.dateTime, event.endDateTime) ? (
            <>
              <div className="flex items-start gap-4 p-4 bg-surface-container rounded-xl">
                ...
                <div>
                  <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-0.5">{t('eventCard.startingDate')}</p>
                  <h3 className="font-bold text-lg text-on-surface leading-tight">{formatDateOnly(event.dateTime, i18n.language)}</h3>
                  <p className="text-sm text-on-surface-variant">{formatTimeOnly(event.dateTime, i18n.language)}</p>
                </div>
              </div>
              <div className="flex items-start gap-4 p-4 bg-surface-container rounded-xl">
                <div className="w-12 h-12 rounded-lg bg-surface-container-highest flex items-center justify-center text-primary flex-shrink-0">
                ...
                <div>
                  <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-0.5">{t('eventCard.endingDate')}</p>
                  <h3 className="font-bold text-lg text-on-surface leading-tight">{formatDateOnly(event.endDateTime, i18n.language)}</h3>
                  <p className="text-sm text-on-surface-variant">{formatTimeOnly(event.endDateTime, i18n.language)}</p>
                </div>
              </div>
            </>
          ) : (
          ...
          >
            ...
            <div className="min-w-0">
              {event.addressLabel ? (
                <>
                  <h3 className="font-bold text-lg text-on-surface leading-tight">{event.addressLabel}</h3>
                  <p className="text-sm text-on-surface-variant">{event.address}</p>
                </>
              ) : (
                <h3 className="font-bold text-lg text-on-surface leading-tight break-words">{event.address}</h3>
              )}
            </div>
          </div>
        </section>
          ...
          <section className={`${PAD} mt-8`}>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-semibold text-on-surface-variant bg-surface-container-high px-3 py-1 rounded-full">
                {event.players.length}{event.maxPlayers ? ` / ${event.maxPlayers}` : ''}
              </span>
            </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 px-1 pt-2 -mx-1">
              ...
              {/* Empty slots */}
              {event.maxPlayers && Array.from({ length: Math.max(0, event.maxPlayers - event.players.length) }).map((_, i) => (
        ...
        {event.description && (
          <section className={`${PAD} mt-8 mb-2`}>
            <h2 className="text-base font-bold text-on-surface mb-3">{t('eventCard.aboutEvent', 'About the Event')}</h2>
