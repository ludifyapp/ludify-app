...
export default function ListingDetailPage() {
  ...
  if (!listing || deleted) return (
    <div className="min-h-screen flex items-center justify-center">
      <p className="text-slate-500 dark:text-zinc-400">{t('marketplace.listingNotFound')}</p>
    </div>
  )
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2.5 pt-2 border-t border-slate-100 dark:border-zinc-800">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs text-slate-400 dark:text-zinc-500">{t('marketplace.listedBy')}</p>
                ...
                {sellerStats && (
                  <p className="text-xs text-slate-400 dark:text-zinc-500 mt-0.5">
                    {sellerStats.hostedCount > 0 ? t('marketplace.hostedCount', { count: sellerStats.hostedCount }) : t('marketplace.newMember')}
                    {sellerStats.memberSince ? t('marketplace.memberSinceShort', { year: sellerStats.memberSince }) : ''}
                  </p>
                )}
              </div>
            </div>
            ...
            {isSeller ? (
              <div className="flex gap-2 pt-2">
                {listing.status === 'active' ? (
                  ...
                  >
                    {actionLoading ? t('marketplace.updating') : t('marketplace.markSold')}
                  </button>
                ) : (
                  <button
                    onClick={relist}
                  ...
                  >
                    {actionLoading ? t('marketplace.updating') : t('marketplace.relist')}
                  </button>
                )}
                <button
              ...
              <div className="space-y-2">
                {user && dms ? (
                  ...
                  >
                    ...
                    {actionLoading ? t('marketplace.opening') : t('marketplace.messageSeller')}
                  </button>
                ) : (
                  <a
